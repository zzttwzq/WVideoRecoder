...
@interface WRecoderVC () <CAAnimationDelegate,WRecorderDelegate,WVideoRecorderControlDelegate>
...
@property (nonatomic,strong) WRecorderService *service;

@end

@implementation WRecoderVC
...
- (void)viewDidLoad {
    ...
    self.service = [WRecorderService service];
...
{
    if (enable) {
        [WRecorderService openFlashLight];
    }
    else{
        [WRecorderService closeFlashLight];
    }
}
...
- (void) recordBtnClickWithSate:(WRecordButtonState)state withMode:(WRecordMode)mode;
{
    ...
    if (mode == WRecordMode_PhotoFromeCamera) {
        ...
        if (state == WRecordButtonState_Recording) {
            ...
            viewc.videoPath = [NSURL URLWithString:self.service.recordFilePath];
